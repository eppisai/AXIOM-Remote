...
void Menu::SetMenuItem(uint8_t index, MenuItem* newMenuItem)
{
...
int8_t Menu::GetMenuItemsCount()
{
    return _menuItemsCount;
}

void Menu::Draw(IPainter* painter)
{
...
void Menu::DrawHeader(IPainter* painter)
{
...
void Menu::DrawMenuItems(IPainter* painter)
{
    int8_t displaySelectionIndex = _menuSelectionIndex - _menuOffset;
...
void Menu::DrawScrollIndicator(IPainter* painter)
{
...
void Menu::UnselectAllMenuItems()
{
    uint8_t b;
...
void Menu::UnhighlightAllMenuItems()
{
    uint8_t b;
    for (b = 0; b < _menuItemsCount; b++)
    {
...
void Menu::Update(ButtonID button, int8_t knob, IMenuSystem* menuSystem)
{
    ...
    case ButtonID::BUTTON_6_UP:
        ...
        break;
    case ButtonID::BUTTON_12_UP:
        SelectionDecrease(menuSystem);
        break;
    case ButtonID::BUTTON_11_UP:
        SelectionPress(menuSystem);
        _menuItem[_menuSelectionIndex]->SetPressed(false);
        break;
    case ButtonID::BUTTON_11_DOWN:
        SelectionPressDown(menuSystem);
        _menuItem[_menuSelectionIndex]->SetPressed(true);
        break;
    case ButtonID::E_1_UP:
        _menuItem[_menuSelectionIndex]->SetPressed(false);
        SelectionPress(menuSystem);
        break;
    case ButtonID::E_1_DOWN:
        SelectionPressDown(menuSystem);
        _menuItem[_menuSelectionIndex]->SetPressed(true);
        break;
    default:
...
void Menu::SelectionIncrease(IMenuSystem* menuSystem)
{
    if (_menuItem[_menuSelectionIndex] == nullptr)
    {
        return;
    ...
    } else if (_popUpParameterMenuActive >= 0)
    {
        ...
        if (_selectionButtonBeingPressed)
        {
            _popUpParameterMenu.SetPressedIndex(_popUpParameterMenu.GetHighlightIndex());
        }
    } else
    {
...
void Menu::SelectionDecrease(IMenuSystem* menuSystem)
{
    if (_menuItem[_menuSelectionIndex] == nullptr)
    {
        return;
...
void Menu::SelectionPressDown(IMenuSystem* menuSystem)
{
...
void Menu::SelectionPress(IMenuSystem* menuSystem)
{
    ...
    } else if (_popUpParameterMenuActive >= 0)
    {
        ...
        _popUpParameterMenu.SetPressedIndex(-1);
    } else
    {
        _menuItem[_menuSelectionIndex]->SetPressed(false);
        ...
        {
            auto currentNumericMenuItem = (NumericMenuItem*)_menuItem[_menuSelectionIndex];
            _numericValueScreen.SetRange(currentNumericMenuItem->GetMinRange(), currentNumericMenuItem->GetMaxRange());
