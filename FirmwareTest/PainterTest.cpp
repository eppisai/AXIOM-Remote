...
TEST_CASE("Framebuffer Swap test")
{
    uint16_t* framebuffer = new uint16_t[15 * 15];
    uint16_t* transitionframebuffer = new uint16_t[15 * 15];
    PainterMod painter(framebuffer, 15, 15);
    memset(framebuffer, 0, 15 * 15);

    painter.DrawIcon(&LogoIcon, 0, 0, 0x7);

    int index = 0, bufferUsed = 0;
    bool check = true;
    ...
    if(index == 60 and check == true){
        bufferUsed++;
    }

    memset(transitionframebuffer, 0, 15 * 15);
    ...
    for (uint16_t yIndex = 0; yIndex < LogoIcon.Height; yIndex++)
    {
        for (uint16_t xIndex = 0; xIndex < LogoIcon.Width; xIndex++)
        {
            if (transitionframebuffer[yIndex * 15 + xIndex] != LogoIconOutput[index])
            {
                check = false;
                break;
            }
